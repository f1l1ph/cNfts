...
contract cNFT is IcNFT {
    ...
    function getLatestOwnerMerkleRoot() external view returns (bytes20) {
        return
            bytes20(concurrentOwnersMerkleRoots[_contractData.concurrentIndex]);
    }
    ...
    function _checkCurrentRootSingle(
        cNFTData memory contractData,
        uint16 nftIndex,
        bytes20 expectedRoot
    ) private view returns (bool) {
        uint8 merkleRootIndex = contractData.concurrentIndex;
        nftIndex++;

        for (uint256 i; i < CONCURRENCY_FACTOR; i++) {
            bytes32 merkleRootData = concurrentOwnersMerkleRoots[
                merkleRootIndex
            ];
    ...
    function has(
        ...
        cNFTData memory contractData = _contractData;
        require(nftIndex < contractData.amount, "Token index out of bounds!");
        bytes20 result = _computeMerkle_calldata(
            bytes20(owner),
            nftIndex,
            proof,
            contractData.depth
    ...
    function transfer(
        ...
        require(nftIndex < contractData.amount, "Token index out of bounds!");
        (
            bytes20 oldAccumulator,
            bytes20 newAccumulator
        ) = _computeMerkle_update_calldata(
                bytes20(msg.sender),
                bytes20(recipient),
                nftIndex,
                proof,
        ...
        require(
            _checkCurrentRootSingle(contractData, nftIndex, oldAccumulator),
            "Merkle root doesn't match!"
        );

        contractData.concurrentIndex = uint8(
            (contractData.concurrentIndex + 1) % CONCURRENCY_FACTOR
        );
        concurrentOwnersMerkleRoots[contractData.concurrentIndex] = bytes32(
            uint256(bytes32(newAccumulator)) | uint256(nftIndex + 1)
        );
    ...
    function transferBatch(
        uint16[] calldata nftIndex,
        address[] calldata recipient,
        bytes calldata proof
    ) external {
        require(nftIndex.length > 0, "List cannot be empty");
        require(
            nftIndex.length == recipient.length,
        ...
        cNFTData memory contractData = _contractData;
        require(
            nftIndex[0] < contractData.amount,
        ...
        require(
            _checkCurrentRoot(contractData, nftIndex, oldAccumulator),
            "Merkle root doesn't match!"
        );
        ...
        for (uint256 i = 1; i < nftIndex.length; i++) {
            (oldAccumulator, newAccumulator) = _computeMerkle_update_calldata(
                bytes20(msg.sender),
                bytes20(recipient[i]),
                nftIndex[i],
                proof,
        ...
        concurrentOwnersMerkleRoots[contractData.concurrentIndex] = bytes32(
            uint256(bytes32(newAccumulator)) | changelog
        );
        _contractData = contractData;
    }

    function _mint(
        string calldata rootDataUrl,
        uint16 amount,
        bytes20 dataRoot,
        bytes20 ownersRoot,
        uint256 depth
    ...
    function mintTransparent(
        ...
        require(
            ...
            "Method can only be called directly by user."
        );
    ...
    function mintTransparentOptimistic(
        string calldata rootDataUrl,
        bytes20 dataRoot,
        bytes20 ownersRoot,
        bytes calldata owners,
    ...
    function _computeMerkle_update_calldata(
        bytes20 leafHash,
        bytes20 newLeafHash,
        uint256 index,
        ...
        uint256 depth,
        uint256 offset
    ) private pure returns (bytes20 oldAccumulator, bytes20 newAccumulator) {
        require(
            merkleProof.length - offset >= (depth - 1) * HASH_WIDTH,
            "Proof incorrect size"
        );
    ...
    function _computeMerkle_calldata(
        bytes20 leafHash,
        uint256 index,
        bytes calldata merkleProof,
        uint256 depth
    ) private pure returns (bytes20 value) {
        require(
            merkleProof.length == (depth - 1) * HASH_WIDTH,
            "Proof incorrect size"
        );
